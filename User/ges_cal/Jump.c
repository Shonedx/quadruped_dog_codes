...
#include "math.h"
#define STRETCH_LENGTH MAX_HEIGHT
#define SHRINK_LENGTH MIN_HEIGHT
#define FALL_LENGTH 12.0f
#define TIME_OFFSET 0.15f //0.15s 前后腿动作时间差

#define JUMP1_Stretch_Length 30
...
void jumpInit(JumpParameter_t *js,byte index)
{
	if(index==1) //jump1
	{
		...
		REP(i,4)
		{
			...
			js->exe_jump_struct[i].stretch_length = JUMP1_Stretch_Length;
	...
	else if (index==2) //jump2
	{
		...
		REP(i,2) //前腿
		{
			...
			js->exe_jump_struct[i].stretch_length = MAX_HEIGHT;
...
byte exeJump(float t, JumpParameter_t *js)
{
	...
	REP(i,2)
	{
		js->exe_jump_struct[i].rotate_angle=js->bend_lean_struct[i].rotate_angle-5;	
		...
		js->exe_jump_struct[i+2].rotate_angle=js->bend_lean_struct[i+2].rotate_angle-5;	
		//后腿跳跃蹬腿时期
	...
	switch (jump_state)
	{
		...
		case JUMP2:
			...
			)
			{
				...
				setMaxSLForSingleLeg(16384,0); //前
				setMaxSLForSingleLeg(16384,2);
				if(jump2_count[0]++>50)
				{
					jump2_exe_flag[0]=exeJumpForSingleLeg(t,js,0,js->exe_jump_struct[0].rotate_angle,js->exe_jump_struct[0].stretch_length, //0
									...
									exeJumpForSingleLeg(t,js,2,js->exe_jump_struct[2].rotate_angle,js->exe_jump_struct[2].stretch_length, //2
														js->bend_lean_struct[2].rotate_angle,js->bend_lean_struct[2].stretch_length);
				}
				...
				if(jump2_count[0]++>50)
				{
					jump2_exe_flag[1]=exeJumpForSingleLeg(t,js,2,js->exe_jump_struct[1].rotate_angle,js->exe_jump_struct[1].stretch_length,
														js->lean_struct[1].rotate_angle,js->lean_struct[1].stretch_length)&&
				...
				)
			{
				...
				if(jump2_count[1]++>50) //rcv
				{
					jump2_rcv_flag[0]=rcvJumpForSingleLeg(t,js,0,js->rcv_jump_struct[0].rotate_angle,js->rcv_jump_struct[0].stretch_length, //0
									...
									rcvJumpForSingleLeg(t,js,1,js->rcv_jump_struct[2].rotate_angle,js->rcv_jump_struct[2].stretch_length, //2
				...
				setMaxSLForSingleLeg(8000,1); //后腿
				setMaxSLForSingleLeg(8000,3);
				// if(jump2_count[3]++>50) //后腿 rcv的延时
				if(jump2_count[1]++>50) //rcv
				
				{
					jump2_rcv_flag[1]=rcvJumpForSingleLeg(t,js,1,js->rcv_jump_struct[1].rotate_angle,js->rcv_jump_struct[1].stretch_length,
